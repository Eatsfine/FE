...
const MenuManagement: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-10">
      ...
      {/* 카테고리 탭 */}
      <div className="flex gap-3 mb-8 p-4 rounded-lg bg-white">
        {categories.map(cat => (
          <button
            key={cat.id}
            onClick={() => setActiveCategory(cat.id as CategoryType)}
            className={`px-5 py-2.5 rounded-lg text-md transition-all ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {filteredMenus.map(menu => (
          <div key={menu.id} className="bg-white border border-gray-100 rounded-lg p-6 transition-all relative group">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-md text-gray-500 leading-relaxed mb-6 line-clamp-2">
                  {menu.description}
                </p>
              </div>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button className="p-2 text-gray-400 hover:text-blue-600 transition-colors"><Pencil size={16} /></button>
            ...
            <div className="flex justify-between items-center mt-auto">
              ...
              <button 
                ...
                className={`w-12 h-6 rounded-full transition-colors relative ${menu.isActive ? 'bg-indigo-600' : 'bg-gray-200'}`}
              >
      ...
      <section className="bg-white border border-gray-100 rounded-lg p-8 shadow-sm">
        <h3 className="text-sm text-gray-400 uppercase tracking-widest mb-6">카테고리 관리</h3>
          ...
          {categories.filter(c => c.id !== 'ALL').map(cat => (
            <div key={cat.id} className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-2xl transition-all group">
              ...
              <div className="flex gap-4 text-sm">
                <button className="text-blue-400 hover:text-blue-600">수정</button>
                <button className="text-red-300 hover:text-red-500">삭제</button>
              </div>
            </div>
          ))}
        </div>
        <button className="w-full mt-4 py-4 border-2 border-dashed border-gray-100 rounded-2xl text-gray-400 text-sm font-bold hover:bg-gray-50 hover:border-blue-200 transition-all flex items-center justify-center gap-2">
