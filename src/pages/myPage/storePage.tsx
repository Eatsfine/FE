...
export default function StorePage() {
  const stats = [
    { label: "총 가게 수", value: "3개", icon: <Store size={20} />, bgColor: "bg-blue-50", iconColor: "text-blue-500" },
    { label: "총 예약 수", value: "2126건", icon: <Calendar size={20} />, bgColor: "bg-indigo-50", iconColor: "text-indigo-500" },
    { label: "평균 평점", value: "4.8", icon: <Star size={20} />, bgColor: "bg-green-50", iconColor: "text-yellow-500" },
  ];
  ...
  const shops = [
    ...
    },
    {
      ...
      rating: "-",
      reviews: "-",
      notice: "가게 승인 대기 중입니다. 영업일 기준 1-2일 내에 승인이 완료됩니다.",
    },
  ];
    ...
    <section className="rounded-xl bg-white p-8 shadow-sm border border-gray-100">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {stats.map((stat, index) => (
          <div key={index} className={cn("flex items-center justify-between p-5 rounded-xl border border-gray-50", stat.bgColor)}>
            <div>
      ...
      <div className="space-y-4 mb-8">
        {shops.map((shop) => (
          <div key={shop.id} className="group relative rounded-2xl border border-gray-200 p-6 hover:shadow-md transition-all cursor-pointer">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={cn(
                      ...
                      shop.status === "운영중" ? "bg-green-50 text-green-600" : "bg-yellow-50 text-yellow-600"
                    )}>
                      {shop.status}
                    </span>
                  </div>
            ...
            {shop.status !== "승인 대기" && (
              <div className="grid grid-cols-3 py-4 border-t border-gray-100 text-center">
                <div>
                  <p className="text-[11px] text-gray-400 mb-1 font-medium">총 예약</p>
                  <p className="text-sm text-gray-700">{shop.totalReservations}</p>
                </div>
                <div className="border-x border-gray-100">
                  <p className="text-[11px] text-gray-400 mb-1 font-medium">평점</p>
                  <p className="text-sm text-gray-700 flex items-center justify-center gap-1">
                    <Star size={14} className="fill-yellow-400 text-yellow-400" /> {shop.rating}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-gray-400 mb-1 font-medium">리뷰</p>
      ...
      <div className="rounded-2xl bg-blue-50/50 p-6 items-center border border-blue-100/50">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-sm text-gray-900">더 많은 데이터가 필요하신가요?</h4>
