export function phoneNumber(value: string) {
  ...
  if (numbers.startsWith("02")) {
    ...
    if (seoulNumbers.length <= 6) return seoulNumbers.replace(/(\d{2})(\d+)/, "$1-$2");
    if (seoulNumbers.length <= 9) return seoulNumbers.replace(/(\d{2})(\d{3})(\d+)/, "$1-$2-$3");
  ...
  if (otherNumbers.length <= 7) return otherNumbers.replace(/(\d{3})(\d+)/, "$1-$2");
  if (otherNumbers.length <= 10) return otherNumbers.replace(/(\d{3})(\d{3})(\d+)/, "$1-$2-$3");
